...
// Initial form values matching your backend model
const initialValues = {
  full_name: "",
  email: "",
  phone_number: "",
...
const ContactAdvancedWithMapBottom = () => {
  ...
  // Form validation schema using Yup (based on your backend requirements)
  const validationSchema = Yup.object({
    ...
    property_type: Yup.string()
      ...
      .max(100, "Property type must be less than 100 characters"),
    ...
    website: Yup.string()
      .max(0, "Spam detected") // Honeypot validation - should always be empty
  });
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      console.log("Form Data to be submitted:", values); // Log form data before submission
      ...
      try {
        // Prepare data for backend (excluding honeypot field)
        const submitData = {
          ...
          // ip_address and user_agent will be captured on the backend
        };
  ...
  // Auto-save draft to localStorage whenever form values change (only when user has interacted)
  useEffect(() => {
    // Only save if form is dirty (user has made changes)
    if (formik.dirty) {
      const saveDraft = () => {
        try {
          localStorage.setItem("anand_contact_draft", JSON.stringify(formik.values));
        } catch (error) {
                  ...
                  <div className="flex-1">
                    ...
                    {submitStatus === "error" && (
                      <div className="bg-rose-50 border border-rose-200 rounded-lg p-4 mb-6">
                        <div className="flex items-center">
                          <div className="w-6 h-6 bg-rose-500 rounded-full flex items-center justify-center mr-3">
                            <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" />
                            </svg>
                          </div>
                          <div>
                            <h4 className="font-semibold text-rose-800 text-sm md:text-base">Submission Failed</h4>
                            <p className="text-rose-600 text-xs md:text-sm">
                    ...
                    <form onSubmit={formik.handleSubmit} className="space-y-6" noValidate>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                        ...
                        />
                        <div>
                          <label htmlFor="property_type" className="block text-sm font-semibold text-slate-700 mb-2">
                            Property Type *
                          </label>
                          <select 
                            id="property_type"
                            ...
                            onChange={formik.handleChange}
                            onBlur={formik.handleBlur}
                            className={`w-full px-3 md:px-4 py-2 md:py-3 bg-slate-50 border ${formik.touched.property_type && formik.errors.property_type ? "border-rose-400" : "border-slate-300"} rounded-lg focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent transition-all duration-300 text-sm md:text-base`} 
                            aria-invalid={formik.touched.property_type && !!formik.errors.property_type}
                          >
                            <option value="">Select property type</option>
                            {propertyTypes.map((t, idx) => <option key={idx} value={t}>{t}</option>)}
                          </select>
                          {formik.touched.property_type && formik.errors.property_type && (
                            <p className="text-rose-600 text-sm mt-2">{formik.errors.property_type}</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                        <div>
                          ...
                          <select 
                            ...
                            onChange={formik.handleChange}
                            onBlur={formik.handleBlur}
                            className={`w-full px-3 md:px-4 py-2 md:py-3 bg-slate-50 border ${formik.touched.budget_range && formik.errors.budget_range ? "border-rose-400" : "border-slate-300"} rounded-lg focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent transition-all duration-300 text-sm md:text-base`} 
                            aria-invalid={formik.touched.budget_range && !!formik.errors.budget_range}
                          >
                            <option value="">Select budget range</option>
                            {budgets.map((b, idx) => <option key={idx} value={b}>{b}</option>)}
                          </select>
                          {formik.touched.budget_range && formik.errors.budget_range && (
                            <p className="text-rose-600 text-sm mt-2">{formik.errors.budget_range}</p>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          <select 
                            ...
                            onChange={formik.handleChange}
                            onBlur={formik.handleBlur}
                            className={`w-full px-3 md:px-4 py-2 md:py-3 bg-slate-50 border ${formik.touched.timeline && formik.errors.timeline ? "border-rose-400" : "border-slate-300"} rounded-lg focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent transition-all duration-300 text-sm md:text-base`} 
                            aria-invalid={formik.touched.timeline && !!formik.errors.timeline}
                          >
                            <option value="">Select timeline</option>
                            {timelines.map((t, idx) => <option key={idx} value={t}>{t}</option>)}
                          </select>
                          {formik.touched.timeline && formik.errors.timeline && (
                            <p className="text-rose-600 text-sm mt-2">{formik.errors.timeline}</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <textarea 
                          ...
                          onBlur={formik.handleBlur}
                          placeholder="Tell us about your specific requirements, preferred locations, amenities, and any other details that will help us find your perfect property..." 
                          className={`w-full px-3 md:px-4 py-2 md:py-3 bg-slate-50 border ${formik.touched.message && formik.errors.message ? "border-rose-400" : "border-slate-300"} rounded-lg focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent transition-all duration-300 resize-none text-sm md:text-base`} 
                          aria-invalid={formik.touched.message && !!formik.errors.message} 
                        />
                        <div className="flex justify-between items-center mt-2">
                          {formik.touched.message && formik.errors.message ? (
                            <p className="text-rose-600 text-sm">{formik.errors.message}</p>
                          ) : (
                            <p className="text-slate-500 text-xs md:text-sm">Please provide detailed information for better assistance</p>
                          )}
                          <p className="text-slate-400 text-xs md:text-sm">{formik.values.message.length}/5000</p>
                        </div>
                      </div>
                      ...
                      <div style={{ display: "none" }} aria-hidden>
                        ...
                        {formik.touched.website && formik.errors.website && (
                          <p className="text-rose-600">{formik.errors.website}</p>
                        )}
                      </div>
                      ...
                      <button 
                        ...
                        className={`w-full py-3 md:py-4 rounded-lg font-bold text-white transition-all duration-300 text-sm md:text-base ${isSubmitting || !formik.isValid ? "bg-slate-400 cursor-not-allowed" : "bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 shadow-lg hover:shadow-xl"}`} 
                        aria-busy={isSubmitting}
                      >
                        {isSubmitting ? (
                          <div className="flex items-center justify-center">
...
function FormField({ id, label, name, type = "text", value, onChange, onBlur, placeholder, error, required }) {
  return (
    <div>
      <label htmlFor={id} className="block text-sm font-semibold text-slate-700 mb-2">{label}</label>
      <input 
